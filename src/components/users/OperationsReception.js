...
const validationSchema = Yup.object().shape({
  ...
  languages: Yup.string().optional().label("languages"),  
});

const OperationsReception = ({ route, navigation }) => {
  const [selected, setSelected] = useState({});

  const receptionApi = useApi(receptionsApi.saveReception);
  const getCountriesApi = useApi(countries.getCountries);
  const [error, setError] = useState();

  useEffect(() => {
    if (route.params) {
      setSelected(route.params.selectedReception);
    }
  ...
  const handleSubmit = async (userInfo) => {
    ...
    try {
      ...
      const result = await receptionApi.request(submitData);
      ...
      <View style={styles.Container}>
        ...
        <KeyboardAwareScrollView>
          ...
          <Navbar
            ...
            Text={`${_id ? "Update" : "Add"} Reception`}
          />

          <ActivityIndicator
            visible={receptionApi.loading || getCountriesApi.loading}
          />
              ...
              >
                ...
                <View>
                  ...
                  >
                    ...
                    >
                      <Icon name="bellreception" size={24} />
