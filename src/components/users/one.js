...
const validationSchema = Yup.object().shape({
  ...
  accountingRegisterNo: Yup.string().optional().label("Accounting-Register-No"),
...
export function OperationsAccountant ({ route, navigation }){
  const [selected, setSelected] = useState({});

  const accountantApi = useApi(accountantsApi.saveAccountant);
  const getCountriesApi = useApi(countries.getCountries);
  const [error, setError] = useState();

  useEffect(() => {
    if (route.params) {
      setSelected(route.params.selectedAccountant);
    }
  ...
  const handleSubmit = async (userInfo) => {
    ...
    try {
      ...
      const result = await accountantApi.request(submitData);
      ...
      <View style={styles.Container}>
        {/* <ActivityIndicator visible={true} />  */}
          ...
          <Navbar
            ...
            Text={`${_id ? "Update" : "Add"} Accountant`}
          />

          <ActivityIndicator
            visible={accountantApi.loading || getCountriesApi.loading}
          />
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <FormField
                      Header={"Accounting-RegisterNo"}
                      img={require("../../../assets/icons/licensed.png")}
                      placeholder="Accounting-RegisterNo"
                      autoCapitalize="none"
                      secureTextEntry={false}
                      isMultiline={false}
                      name="accountingRegisterNo"
                    />
                    <FormField
                      Header={"License No"}
                      img={require("../../../assets/icons/licensed.png")}
